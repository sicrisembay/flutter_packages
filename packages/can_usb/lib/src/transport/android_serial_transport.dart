/// [AndroidSerialTransport] — [ISerialTransport] implementation for Android.
library;
...
/// Both null [_port] so [isConnected] returns `false` at the next watchdog tick.
class AndroidSerialTransport implements ISerialTransport {
  // Broadcast stream controller — kept alive across connections.
  ...
  Future<void> connect(String portName, {int baudRate = 115200}) async {
    ...
    // detach racing with connect() is still observed.
    ...
    final device = devices.firstWhere(
      ...
      orElse: () => throw CanConnectionException(
        'USB device not found: $portName. Ensure the device is still connected.',
      ),
    );
